...
const ProjectChat = ({ project }) => {
  const { user } = useAuth();
  const { fetchProjectMessages, sendExistingProjectMessage } = useProjects();
  ...
  const memberIds = useMemo(() => (project.members || []).map((member) => toId(member)), [project.members]);
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      const data = await fetchProjectMessages(project._id);
    ...
    socket.on('connect', () => {
      setConnected(true);
      socket.emit('project:join', { projectId: project._id }, (response) => {
    ...
    socket.on('project:message', (message) => {
      if (!sameId(message.project, project._id)) return;
  ...
  const handleSubmit = async (e) => {
    ...
    if (socket?.connected) {
      socket.emit('project:message', { projectId: project._id, body: text }, (response) => {
        if (!response?.success) {
    ...
    try {
      const message = await sendExistingProjectMessage(project._id, text);
  ...
  if (!canChat) {
    return (
      <div className="card p-4 md:p-8 border-l-4 border-l-zinc-400">
        <h2 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">Project Chat</h2>
        <p className="text-zinc-600 dark:text-zinc-400">Chat is available after the owner approves your application.</p>
      </div>
    );
  }
  ...
  return (
    <div className="card p-4 md:p-8 border-l-4 border-l-green-600">
      <div className="flex items-center justify-between gap-3 mb-4">
        <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">Project Chat</h2>
